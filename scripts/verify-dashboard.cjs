...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.expenses-brasil .expense-details').getAttribute('open'), null);
    assert.doesNotMatch(await page.locator('.expenses-brasil .expense-legend-row[data-category="consortium"]').innerText(), /R\$/);
    await page.locator('.expenses-brasil .expense-details summary').click();
    assert.match(await page.locator('.expenses-brasil .expense-details').innerText(), /Caixa/);
    await page.locator('.expenses-brasil .expense-details summary').click();
    assert.equal(await page.evaluate(() => window.testApp.getState().ui.hideRecentTransactions), true);
    assert.match(await page.locator('.recent-transactions-panel').innerText(), /Lançamentos ocultos/);
    await page.locator('.recent-transactions-panel .visibility-toggle').click();
    assert.equal(await page.evaluate(() => window.testApp.getState().ui.hideRecentTransactions), false);
    await page.locator('.recent-transactions-panel .visibility-toggle').click();
    assert.equal(await page.evaluate(() => window.testApp.getState().ui.hideRecentTransactions), true);
    ...
    const desktopOrder = await page.evaluate(() => window.testApp.getState().ui.dashboardLayouts.desktopFlow);
    assert.ok(desktopOrder.indexOf('vehicle-panel') > desktopOrder.indexOf('housing-panel'));
    await page.reload();
    assert.ok(await page.locator('[data-dashboard-card="vehicle-panel"]').evaluate(el => el.parentElement.classList.contains('dashboard-masonry-list')));
    // Exercise pointer drag across the balanced columns as well as keyboard ordering.
    ...
    await page.mouse.up();
    assert.ok(await page.locator('[data-dashboard-card="overview-card"]').evaluate(el => el.parentElement.classList.contains('dashboard-masonry-list')));
    const savedDesktop = await page.evaluate(() => JSON.stringify(window.testApp.getState().ui.dashboardLayouts.desktopFlow));
    ...
    assert.equal(await page.evaluate(() => JSON.stringify(window.testApp.getState().ui.dashboardLayouts.desktopFlow)), savedDesktop);
    await page.reload();
