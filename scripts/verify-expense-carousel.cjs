...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.emergency-reserve-panel .pro-summary-grid').count(), 0);
    assert.equal(await page.locator('.emergency-card').getAttribute('data-reserve-progress'), '0');
    assert.equal(await page.locator('.emergency-progress').getAttribute('aria-valuenow'), '0');
    assert.equal(await page.locator('.emergency-card').evaluate(el => el.style.getPropertyValue('--reserve-hue')), '0');
    ...
    assert.ok(Math.max(...bottoms) - Math.min(...bottoms) < 2, 'All group backgrounds extend to the same bottom');
    assert.equal(await page.locator('.account-swipe-card.has-bank-art.is-active').evaluate(el => getComputedStyle(el).boxShadow), 'none');
    const firstHandle = page.locator('[data-dashboard-group="investments"] [data-dashboard-card]').first().locator('.dashboard-handle');
    await firstHandle.focus();
    await firstHandle.press('ArrowDown');
    const investmentOrder = await page.locator('[data-dashboard-group="investments"] [data-dashboard-card]').evaluateAll(nodes => nodes.map(node => node.dataset.dashboardCard));
    await page.evaluate(() => window.testApp.renderKeepingScroll());
    assert.deepEqual(await page.locator('[data-dashboard-group="investments"] [data-dashboard-card]').evaluateAll(nodes => nodes.map(node => node.dataset.dashboardCard)), investmentOrder);
